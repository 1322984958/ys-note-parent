layui.define(['form', 'laypage'], function (exports) {
    var form = layui.form,
        Grid = {
            ...
            createNew: function (option) {
                ...
                var grid = {
                    elem: typeof (option.elem) === 'string' ? $('#' + option.elem) : option.elem, // GridID
																									// 或
																									// Jquery对象
                ...
                function fnHeader(html, content) {
                    ...
                    tbHead.find('thead th,thead td').each(function (i) {
						 ...
						 * that.append('<i class="grid-order"></i>').on('click',
						 * events.order); }
						 * 
						 * if (i == 0 && $(this).find(':checkbox').length > 0 &&
						 * !grid.singleSelect) {
                ...
                grid.selectRow = function () {
                    /*
					 * if (values != undefined) { grid.elem.find('.grid-body
					 * tbody tr.selected').removeClass('selected').each(function () {
					 * if ($(this).find('td:first :checkbox').length > 0)
					 * $(this).find('td:first :checkbox')[0].checked = false;
					 * }); var keys = grid.keys.split(';'); if
					 * (Object.prototype.toString.call(values) == '[object
					 * Array]' && !grid.singleSelect) { $.each(values, function
					 * (a, value) { $.each(grid.pageData.rows, function (i, r) {
					 * var key = ''; $.each(keys, function (j, k) { key +=
					 * r[k]+';'; }); if (value + ';' == key) { var tr =
					 * grid.elem.find('.grid-body tbody tr:eq(' + i + ')');
					 * tr.addClass('selected'); if (tr.find('td:first
					 * :checkbox').length > 0) tr.find('td:first
					 * :checkbox')[0].checked = true; return false; } }); }); }
					 * else { $.each(grid.pageData.rows, function (i, r) { var
					 * key = ''; $.each(keys, function (j, k) { key += r[k]+';';
					 * }); if (values + ';' == key) { var tr =
					 * grid.elem.find('.grid-body tbody tr:eq(' + i + ')');
					 * tr.addClass('selected'); if (tr.find('td:first
					 * :checkbox').length > 0) tr.find('td:first
					 * :checkbox')[0].checked = true; return false; } }); } }
    ...
    exports('grid', Grid);
    layui.link(layui.cache.base + '../../libs/layui/extend/css/grid.css', null, 'gridcss');
});
