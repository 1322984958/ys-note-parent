$(function() {
	...
	layui.table.render({
		...
		cols: [[
			...
			{field:'orders', sort: true, title: '排序'},
	...
	layui.form.on('submit(btnSubmit)', function(data){
		...
		layer.load(1);
		$.post("../api/role/save", data.field, function(data){
...
function doDelete(obj){
	layer.confirm('确定要删除吗？', function(index){
		...
		layer.load(1);
		$.ajax({
			url: "../api/role/del/"+obj.data.id+"?token="+getToken(), 
			type: "DELETE", 
			dataType: "JSON", 
...
function showPermDialog(roleId){
	...
	$.post("../api/permission/tree/"+roleId,{
		token: getToken()
	},function(data) {
...
function saveRolePerm(roleId,index){
	...
	$.post("../api/permission/tree/saveRolePermissions",{
		roleId: roleId,
		permIds: ids,
		token: getToken(),
		_method: "PUT"
